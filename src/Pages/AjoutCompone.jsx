...
function AjoutCompone() {
  ...
  const handleImageUpload = async (e, setImgState, setLoading = null) => {
    ...
    try {
      // Options de compression (tu peux les ajuster ou les rendre configurables)
      const options = {
        maxSizeMB: 1,                // Taille max finale ≈ 1 Mo
        maxWidthOrHeight: 1200,      // Redimensionne si plus grand
        useWebWorker: true,          // Ne bloque pas l'UI
        fileType: 'image/jpeg',      // Force JPEG (plus léger), ou garde original avec undefined
        quality: 0.8,
        initialQuality: 0.9,
      };
      ...
      // Ou si tu veux un File avec le nom original :
      // const compressedFileWithName = new File([compressedFile], file.name, { type: compressedFile.type });
  ...
  return (
    <>
      <div style={{ padding: 0, width: "70%", margin: "auto" }} >
          ...
          <div className="text-center lg:mt-[5%] mt-10 mb-2 sm:px-0 px-[5%]">
            ...
            >
              ...
              <input
                ...
                ref={fileInputRef}
                accept="image/*"
                onChange={(e) => handleImageUpload(e, setCertificateImg, setLoading)}
                className="hidden"
              />
              <button
                type="button"
          ...
          <div className="text-center lg:mt-[5%] mt-10 mb-2 sm:px-0 px-[5%]">
            ...
            >
              <label className="block text-sm font-medium text-white">
                ...
                {loading && <p>Compression en cours...</p>}
              </label>
              {imageBase64 && <div><img src={imageBase64} alt="Aperçu" className="w-20 h-20 rounded-full m-auto" /></div>}
              <input
                type="file"
                accept="image/*"
                onChange={(e) =>
                  handleImageUpload(e, (img) => setProject({ ...project, img }), setLoading)
                }
                ref={fileInputRefZ}
                className="hidden"
              />
